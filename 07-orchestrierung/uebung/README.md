...
Ziel dieser Übung ist es, den Microservice aus Übung 1 mit Kubernetes zu betreiben.
...
* nutzen sie die Lösung der [Übung 06](../../06-cluster-scheduling/uebung_aws/loesung/book-service)
* bauen sie dafür das Docker Image und prüfen sie die lokale Lauffähigkeit
* loggen sie sich bei Docker Hub ein
* pushen sie das funktionsfähige Image nach Docker Hub

```bash
#Build image
mvn clean package && sudo docker build -t <dockerhub-user>/book-service:latest .
#login to Docker hub
sudo docker login
#Push to Docker Hub
sudo docker push <dockerhub-user>/book-service:latest
```
...
Rufen sie anschließend den deployten Service über die ip der Minikube node und
den Port des NodePort Services auf.
Die Ip der cluster node erhalten sie mit
```bash
minikube ip
```
Den Port des NodePort Service erhalten sie mit
```bash
